...
$(document).ready(function () { 
    ...
    $('.license-img > img').on('click', function(){

            var openImg = $(this).attr('src');
            
            $('.license-wrapper').prepend('<div id="modal_license" style="position: fixed; z-index: 10000; width: 100%; height: 100%; background-color: rgba(0,0,0,0.7); display: none; justify-content: center; align-items: center;"><div id="modal_license_img_wrapper" style="position: relative; display:none;"><img class="license_big" width="40vw" height="auto"><div id="modal_license_close" style="position: absolute; z-index: 10001; width: 25px; height: 25px; top: 5px; right: 5px; cursor: pointer; background-color: #fff; border-radius: 100%; overflow: hidden;"><img src="img/svg/close.svg"></div></div></div>');

            $('.license_big').attr('src', openImg);
    });
       ...
       /*   
        ...
        $('.license_big').
        $('#modal_license').delay(800).fadeIn();
        setTimeout(function () {
            $('#modal_license').css('display', 'flex');
        }, 1000);

    });

        $('#modal_license_img_wrapper').delay(1200).fadeIn('slow');
        $('#modal_license_close').on('click', function () {
            $('#modal_license').delay(300).fadeOut();        
        }); 
    });*/